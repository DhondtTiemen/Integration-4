<template>
    <div>
      ...
      <div v-else>
        ...
        <div v-else class="text-gray-500 p-4">Not follow anyone yet.</div>
      </div>
    </div>
  </template>
  ...
  onMounted(async () => {
    ...
    const followIds: number[] = currentUser?.followers || [];
  
    // Map de follow naar user info, fallback naar "Unknown"
    followList.value = followIds.map((fid: number) => {
