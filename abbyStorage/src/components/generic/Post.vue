...
<!-- Posts -->
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="relative">
          ...
          <div v-if="showOptionsId === post.id" class="absolute top-6 right-0 bg-white border shadow-md z-10">
            <button @click="emit('report-post', post.id)" class="block px-4 py-2 text-red-600 hover:bg-gray-100">Report Post</button>
          </div>
        </div>
      </div>
    ...
    <router-link :to="`/post/${post.id}`" v-if="post.eventId && event" class="mb-2 relative">
...
<script setup>
    ...
    const user = computed(() => {
        return props.post ? users.users.find(u => u.id === props.post.userId) : null
    })

    const event = computed(() => {
        return props.post?.eventId ? events.events.find(e => e.id === props.post.eventId) : null
    })
