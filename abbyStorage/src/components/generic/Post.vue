<template>
  ...
  <div class="p-4 border-b-[1px] border-gray-200">
    ...
    <div class="flex items-center justify-between mb-6">
      <router-link :to="`/account/${user?.id}`" class="flex items-center space-x-3">
        <img :src="user?.avatar" alt="avatar" class="w-12 h-12 rounded-full mr-6" />
        <div>
      ...
      <div class="relative">
        ...
        <div v-if="showOptionsId === post?.id" class="absolute top-6 right-0 bg-white border shadow-md z-10">
          <button @click="emit('report-post', post?.id)" class="block px-4 py-2 text-red-600 hover:bg-gray-100">
            Report Post
          </button>
        </div>
      </div>
    ...
    <router-link :to="`/post/${post?.id}`" class="block mb-6">
      ...
      <div v-if="post?.images.length === 1" class="mb-6">
        ...
        <img :src="'https://images.pexels.com/photos/7585762/pexels-photo-7585762.jpeg'" alt="post image"
  ...
  <router-link :to="`/event/${event?.id}`" v-if="post?.eventId && event" class="mb-2 relative">
    <span class="absolute top-7 right-4 border border-black text-black text-xs px-2 py-1">Event</span>
    <template v-if="event">
      <img :src="'https://images.pexels.com/photos/7585762/pexels-photo-7585762.jpeg'" alt="post image"
      ...
      <div class="bg-purple-300 p-4">
        ...
        <div class="my-2">
          <p class="text-sm flex items-center gap-2 mb-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
              ...
              <path d="M12.8069 2.41138V0H11.2107V2.41138H4.78935V0H3.19315V2.41138H0V5.60184H16V2.41138H12.8069Z"
                fill="black" />
            </svg>
        ...
        <button class="mt-3 bg-amber-300 text-black font-medium w-full py-1.5 rounded">
          Learn more
        </button>
      </div>
    </template>
...
function hasLikedPost() {
  return props.post?.likes && props.post.likes.includes(myUserId);
}

function togglePostLike() {
  if (!props.post) return;
  if (!props.post.likes) props.post.likes = [];
  const idx = props.post.likes.indexOf(myUserId);
  if (idx === -1) {
    props.post.likes.push(myUserId);
  } else {
