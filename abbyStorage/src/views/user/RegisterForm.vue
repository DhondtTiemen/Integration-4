<template>
  <section class="min-h-screen flex items-center justify-center px-6 py-8 font-primary bg-alphaYellow">
    <div class="w-full max-w-md">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-auto mx-auto mb-8" viewBox="0 0 71 24" fill="none">
        <path
      ...
      <div class="p-6 space-y-6">
        <div class="mb-8">
          ...
          <p class="text-center">Sign up to get started</p>
        </div>

        <form @submit.prevent="handleRegister" class="space-y-4">
          <div>
            <label for="name" class="block mb-2 text-sm">Username</label>
            <input v-model="name" type="name" id="name" placeholder="JohnD" required
          ...
          <div>
            ...
            <input v-model="email" type="email" id="email" placeholder="johndoe@email.com" required
              class="border border-alphaDark text-gray-600 text-sm block w-full p-2.5 focus:ring-alphaDark focus:border-alphaDark" />
          </div>
          <div>
          ...
          <div>
            <label for="confirmPassword" class="block mb-2 text-sm">Confirm password</label>
            <input v-model="confirmPassword" type="password" id="confirmPassword" placeholder="••••••••" required
              class="border border-alphaDark text-gray-600 text-sm block w-full p-2.5 focus:ring-alphaDark focus:border-alphaDark" />
          </div>
          <div class="flex items-start">
            <div class="flex items-center h-5">
              <input v-model="acceptedTerms" id="terms" type="checkbox" required
                class="appearance-none w-4 h-4 border border-alphaDark focus:ring-3 focus:ring-alphaDark checked:bg-alphaGreen checked:border-alphaGreen transition-colors" />
            </div>
            <div class="ml-3 text-sm">
              <label for="terms" class="font-light text-alphaDark">
                I accept the
                <a href="https://www.kortrijk.be/privacyverklaring"
                  class="font-medium text-alphaDark hover:underline dark:text-primary-500">Privacy Policy</a>
              </label>
            </div>
          </div>
          <div class="flex items-start mb-6">
            <div class="flex items-center h-5">
              <input v-model="subscribeNewsletter" id="newsletter" type="checkbox"
                class="appearance-none w-4 h-4 border border-alphaDark p-1 focus:ring-3 focus:ring-alphaDark checked:bg-alphaGreen checked:border-alphaGreen transition-colors" />
            </div>
            <div class="ml-3 text-sm">
              <label for="newsletter" class="font-light text-alphaDark">
                I want to subscribe to the
                <span class="font-medium text-alphaDark hover:underline dark:text-primary-500">newsletter</span>
              </label>
            </div>
          </div>

          <button type="submit"
            class="w-full bg-alphaGreen focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium px-5 py-2.5">
            Create your account
          </button>
          <p class="text-sm font-light">
            Already have an account?
            <router-link to="/login" class="font-medium text-alphaGreen underline dark:text-primary-500">Login
              here</router-link>
          </p>
        </form>
      </div>
...
async function handleRegister() {
  ...
  const user: User = {
    ...
    bio: "",
    avatar: "./src/assets/users/default.png", aboutMe: "",
    followers: [],
    following: [],
    achievements: [],
    box: {
