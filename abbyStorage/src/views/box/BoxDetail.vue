  ...
  <div class="min-h-screen mb-16">
    ...
    <div v-else>
      ...
      <div v-if="user && user.box && user.box.boxNumber != 0" v-bind="$attrs">
        <div v-if="storedIdRaw != currentUserId"
        ...
        <div class="p-4 border-b-2 border-gray-200">
          ...
          <div v-if="user.box.items.length > 0" class="grid grid-cols-2 gap-4 overflow-y-auto">
            <div v-for="(item, index) in (user?.box?.items as BoxItem[])" :key="index">
              <div>
      ...
      <div v-else-if="!loading" class="p-4 text-center flex mt-64 justify-center text-gray-600">
        <p class="text-lg font-semibold">
          {{ user?.name }} did not created a box yet
        </p>
      </div>
        ...
        <div class="flex gap-4 items-start mt-4" v-for="(comment, index) in comments" :key="index">
          ...
          <div>
            ...
            <div class="flex items-center gap-2 mt-2 text-sm">
              <button class="flex items-center gap-1" @click="toggleCommentLike(comment, index)"
