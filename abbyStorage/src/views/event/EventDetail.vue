  ...
  <div class="min-h-screen bg-white flex flex-col pb-16">
    ...
    <section v-if="event" class="p-4 mb-4 flex flex-col gap-4">
      ...
      <div>
        ...
        <p v-if="event.status == 'Pending'" class="text-sm text-gray-600 flex mt-1 items-center gap-1">
          <Clock class="w-4 h-4" />
          {{ event.status }}
        </p>
        ...
        <button class="mt-3 px-5 py-2.5 w-full text-sm font-medium transition-all" :class="[
          isPast
            ? 'bg-gray-400 cursor-not-allowed'
            : hasParticipate()
              ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
              : 'bg-alphaGreen hover:bg-green-700',
        ]" :disabled="isPast" @click="toggleParticipateEvent">
          ...
          <div class="flex -space-x-3">
            <template v-for="(p, i) in event.participants.slice(0, 6)" :key="i">
              <template v-if="getUserInfo(p)?.name === 'Unknown'">
                <CircleUserRound class="w-10 h-10 rounded-full border-2 border-white bg-gray-200 text-gray-600" />
              </template>
              <template v-else>
                <img :src="getUserInfo(p)?.avatar" class="w-10 h-10 rounded-full border-2 border-white"
                  :alt="getUserInfo(p)?.name" />
              </template>
            </template>
            <div v-if="event.participants.length > 6"
      ...
      <div>
        ...
        <div class="flex justify-center mt-4 items-center gap-4 p-3">
          <div class="flex flex-col items-center text-xs">
            ...
            <svg width="52" height="52" viewBox="0 0 52 52" class="fill-alphaBlue mb-2"
      ...
      <div>
        ...
        <ul>
          <li v-for="(m, index) in event.materials" :key="index" class="flex items-center gap-2 mb-1">
      ...
      <div v-if="isPast">
        ...
        <div v-if="gallery.length === 0">
          <div v-if="event?.createdBy === storedIdRaw" class="text-center text-gray-500 py-4">
            <button class="bg-alphaGreen text-white px-4 py-2 rounded font-medium" @click="triggerFileInput"
              :disabled="uploading">
              Add photos
            </button>
            <input ref="fileInput" type="file" accept="image/*" multiple class="hidden" @change="handleGalleryUpload" />
        ...
        <div v-else class="grid grid-cols-4 gap-2">
          <div v-for="(img, i) in gallery.slice(0, 3)" :key="i" class="aspect-square bg-gray-200">
          ...
          <div v-if="event?.createdBy === storedIdRaw" class="col-span-4 text-center mt-2">
            <button class="bg-alphaGreen text-white px-4 py-2 rounded font-medium" @click="triggerFileInput"
              :disabled="uploading">
              Add more photos
            </button>
      ...
      <div>
        ...
        <div v-else class="space-y-4">
          <router-link :to="`/event/${related.id}`" v-for="related in relatedEvents.slice(0, 3)" :key="related.id"
            ...
            <button class="bg-alphaYellow w-fit px-4 py-1 mt-2 text-sm font-medium">
              Learn more
            </button>
          </router-link>
          <router-link v-if="relatedEvents.length > 3" :to="`/events`"
